...
<head>
    ...
    <title><fmt:message key="orders.title_page"/></title>
    ...
    <div class="container">
        ...
        <div class="d-flex align-items-center gap-2">
            <a href="${pageContext.request.contextPath}/cart" class="btn btn-outline-light btn-sm">
                <i class="bi bi-cart3 me-1"></i> <fmt:message key="orders.nav_cart"/>
            </a>
        </div>
    </div>
</nav>
...
<div class="container my-4 flex-grow-1">
    <h3 class="fw-bold mb-4"><i class="bi bi-box-seam me-2"></i><fmt:message key="orders.heading"/></h3>
    ...
    <ul class="nav nav-tabs mb-4 bg-white rounded shadow-sm px-3 pt-2 border-0 overflow-x-auto flex-nowrap">
        <li class="nav-item">
            <a class="nav-link ${empty activeTab || activeTab == 'all' ? 'active fw-bold text-primary border-bottom border-primary border-3' : 'text-dark'}"
               href="${pageContext.request.contextPath}/orders?tab=all"><fmt:message key="orders.tab_all"/></a>
        </li>
        <li class="nav-item">
            <a class="nav-link ${activeTab == 'pending_payment' ? 'active fw-bold text-primary border-bottom border-primary border-3' : 'text-dark'}"
               href="${pageContext.request.contextPath}/orders?tab=pending_payment"><fmt:message key="orders.tab_pending_payment"/></a>
        </li>
        <li class="nav-item">
            <a class="nav-link ${activeTab == 'pending_ship' ? 'active fw-bold text-primary border-bottom border-primary border-3' : 'text-dark'}"
               href="${pageContext.request.contextPath}/orders?tab=pending_ship"><fmt:message key="orders.tab_pending_ship"/></a>
        </li>
        <li class="nav-item">
            <a class="nav-link ${activeTab == 'shipping' ? 'active fw-bold text-primary border-bottom border-primary border-3' : 'text-dark'}"
               href="${pageContext.request.contextPath}/orders?tab=shipping"><fmt:message key="orders.tab_shipping"/></a>
        </li>
        <li class="nav-item">
            <a class="nav-link ${activeTab == 'completed' ? 'active fw-bold text-primary border-bottom border-primary border-3' : 'text-dark'}"
               href="${pageContext.request.contextPath}/orders?tab=completed"><fmt:message key="orders.tab_completed"/></a>
        </li>
        <li class="nav-item">
            <a class="nav-link ${activeTab == 'returned' ? 'active fw-bold text-primary border-bottom border-primary border-3' : 'text-dark'}"
               href="${pageContext.request.contextPath}/orders?tab=returned"><fmt:message key="orders.tab_returned"/></a>
        </li>
    </ul>
            ...
            <c:choose>
                <c:when test="${empty orders}">
                    <div class="card border-0 shadow-sm p-5 text-center bg-white rounded-3 my-5">
                        ...
                        <h5 class="text-secondary"><fmt:message key="orders.empty_title"/></h5>
                        <p class="text-muted"><fmt:message key="orders.empty_desc"/></p>
                        <div>
                            <a href="${pageContext.request.contextPath}/home" class="btn btn-primary px-4 mt-2">
                                <fmt:message key="orders.btn_shop_now"/>
                            </a>
                        </div>
                    </div>
                </c:when>
                    ...
                    <c:forEach var="order" items="${orders}">
                        <div class="card border-0 shadow-sm rounded-3 mb-3 p-4 bg-white">
                            ...
                            <div class="d-flex justify-content-between align-items-center border-bottom pb-2 mb-3">
                                <span class="fw-bold text-secondary"><fmt:message key="orders.order_id"/>: #${order.orderID}</span>
                            ...
                            <c:forEach var="detail" items="${order.orderDetails}">
                                <div class="d-flex align-items-center mb-3">
                                    ...
                                    <div class="flex-grow-1">
                                        ...
                                        <div class="text-muted small"><fmt:message key="orders.qty_prefix"/>: x${detail.quantity}</div>
                                    </div>
                            ...
                            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                                <div>
                                    <span class="text-muted"><fmt:message key="orders.total_label"/>: </span>
                                ...
                                <div>
                                    <c:choose>
                                        ...
                                        <c:when test="${order.status.name() == 'Pending' || order.status.name() == 'PENDING_PAYMENT'}">
                                            <a href="${pageContext.request.contextPath}/checkout?orderId=${order.orderID}" class="btn btn-primary btn-sm fw-bold px-3">
                                                <fmt:message key="orders.btn_pay_now"/>
                                            </a>
                                        </c:when>
                                        ...
                                        <c:when test="${order.status.name() == 'Paid'}">
                                            <button class="btn btn-secondary btn-sm fw-bold px-3" disabled>
                                                <fmt:message key="orders.btn_preparing"/>
                                            </button>
                                        </c:when>
                                        ...
                                        <c:when test="${order.status.name() == 'Shipping' || order.status.name() == 'SHIPPING'}">
                                            <button class="btn btn-success btn-sm fw-bold px-3">
                                                <fmt:message key="orders.btn_received"/>
                                            </button>
                                        </c:when>
                                        ...
                                        <c:when test="${order.status.name() == 'Delivered' || order.status.name() == 'COMPLETED'}">
                                            <button class="btn btn-outline-primary btn-sm fw-bold px-3">
                                                <fmt:message key="orders.btn_review"/>
                                            </button>
                                        </c:when>
                                        ...
                                        <c:when test="${order.status.name() == 'Cancelled'}">
                                            <span class="text-danger small fw-semibold">
                                                <i class="bi bi-x-circle me-1"></i><fmt:message key="orders.msg_cancelled"/>
                                            </span>
                                        </c:when>
                                    </c:choose>
                                </div>
